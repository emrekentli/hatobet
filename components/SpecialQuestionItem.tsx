...
const SpecialQuestionItem: React.FC<SpecialQuestionItemProps> = ({
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded p-3 bg-white dark:bg-gray-800">
      ...
      {/* Soru tipi ve cevap alanı */}
      {hasAnswered && !match.isActive && !match.isFinished ? (
        // Maç başlamadı, cevap var => Input alanı açık, önceki cevap inputta
        <div>
          ...
          <button
            onClick={() => handleAnswerQuestion(question.id, questionAnswers[question.id] || "")}
        ...
        // Maç başladı veya bitti, sadece cevabı göster
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400">Maç bitmiş, cevap veremezsiniz.</div>
      ) : (
        // Hiç cevap yok, maç başlamamış
        <div>
          {question.questionType === "MULTIPLE_CHOICE" && (
            <div className="space-y-1">
              {question.options?.map((option: string, idx: number) => (
